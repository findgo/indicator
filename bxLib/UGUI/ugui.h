...
#elif defined(USE_COLOR_MONOCHROME)
...
typedef struct
{
   void (*pset)(UG_S16,UG_S16,UG_COLOR);  // set pixel on screen
   UG_S16 x_dim;    // Dimension x
   UG_S16 y_dim;    // Dimension y
   UG_TOUCH touch;  // touch key?
   UG_WINDOW* next_window; // next window
   UG_WINDOW* active_window; // active window
   UG_WINDOW* last_window;   // last window
   struct
   {
      UG_S16 x_pos;
      UG_S16 y_pos;
   ...
   UG_FONT font;
   UG_S8 char_h_space;  // char the space for horizontal
   UG_S8 char_v_space;   // char the space for vertical 
   UG_COLOR fore_color;
   UG_COLOR back_color;
   UG_COLOR desktop_color;
   UG_U8 state;   // State Machine
